...
ob_start();
$time_ack = time(); //## EMAIl ACK  ##
require_once dirname(__FILE__).'/include/page_header.php';
require_once dirname(__FILE__).'/include/tr_email_ack.inc.php'; //## EMAIl ACK  ##
...
if (isset($_REQUEST['save']) || isset($_REQUEST['saveandreturn'])) {
	...
	if (isset($_REQUEST['saveandreturn'])) {
		while($ev_open = DBfetch($evs_open) ){
                                send_email_ack($ev_open['objectid'], $ev_open['min(e.eventid)'],$ev_open['min(e.clock)'], $time_ack,getSeverityCaption($ev_open['priority']),  $USER_DETAILS['name']." ".$USER_DETAILS['surname'] );//## EMAIl ACK  ##
        }
		ob_end_clean();
		if ($_REQUEST['backurl'] == 'tr_events.php') {
			redirect($_REQUEST['backurl'].'?eventid='.$_REQUEST['eventid'].'&triggerid='.$_REQUEST['triggerid']);
